...
struct PieChart: View {
  ...
  init(data: [Double], labels: [String], colors: [Color], borderColor: Color) {
    ...
    self.labels = labels
    self.colors = colors
    if let firstIndex = self.data.firstIndex(of: 0.0) {
      self.data.remove(at: firstIndex)
      self.labels.remove(at: firstIndex)
      self.colors.remove(at: firstIndex)
    }
    ...
    GeometryReader { geo in
      ZStack(alignment: .center) {
        ForEach(0 ..< data.count) { index in
          ...
          PieSliceText(
            title: String(format: "%.0f \(data[index] > 1 ? "Answers": "Answer")", data[index]),
